...
export interface Finding {
  /** ISO timestamp. */
  ts: string
  /** Iteration number when the finding was made. */
  iteration: number
  /** Short description of the finding. */
  description: string
  /** Evidence (paths, citations, experiment ids). */
  evidence: string[]
  /** Direction label under which this finding was produced. */
  direction: string
}

/** A direction that has been tried, for diversity enforcement. */
export interface DirectionTried {
  ts: string
  iteration: number
  /** Short label for the direction. */
  label: string
  /** Hypothesis or strategy description. */
  hypothesis: string
  /** Outcome: 'progress' | 'stall' | 'fail'. */
  outcome: 'progress' | 'stall' | 'fail'
}
...
export interface ProgressState {
  iteration: number
  status: 'pending' | 'in_progress' | 'stalled' | 'done' | 'failed'
  staleCount: number
...
type Op =
  | { op: 'read' }
  | { op: 'write_progress'; progress: Partial<ProgressState> }
  | { op: 'append_finding'; finding: Omit<Finding, 'ts'> }
  | { op: 'append_direction'; direction: Omit<DirectionTried, 'ts'> }
  | { op: 'append_log'; entry: Omit<IterationLogEntry, 'ts'> }
  | { op: 'write_spec'; content: string }

export interface StateOutput {
  op: string
  progress: ProgressState | null
  findings: Finding[]
  directions: DirectionTried[]
  iterationLog: IterationLogEntry[]
  /** Omitted from the canonical JSON snapshot when the spec file does not exist. */
  taskSpec?: string
...
export function apply(ctx: Context, config: Config): void {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      const applyOp = async (): Promise<void> => {
        switch (op.op) {
          case 'write_spec': {
          ...
          case 'write_progress': {
            const current = await readJsonIfExists<ProgressState>(
          ...
          case 'append_finding': {
            const finding: Finding = { ...op.finding, ts: now }
          ...
          case 'append_direction': {
            const dir: DirectionTried = { ...op.direction, ts: now }
            await appendJsonl(join(stateDir, 'directions_tried.json'), dir)
            break
          }
          case 'append_log': {
            const entry: IterationLogEntry = { ...op.entry, ts: now }
      ...
      const progress = await readJsonIfExists<ProgressState>(
        join(stateDir, 'progress.json'), DEFAULT_PROGRESS)
      const findings = await readJsonlIfExists<Finding>(join(stateDir, 'findings.jsonl'))
      const directions = await readJsonlIfExists<DirectionTried>(join(stateDir, 'directions_tried.json'))
      const iterationLog = await readJsonlIfExists<IterationLogEntry>(join(logsDir, 'iteration_log.jsonl'))
    ...
    presentCall: (args) => {
      ...
      return {
        card: 'generic',
        kind: 'other',
        title: OP_LABELS[(args.op as { op: string }).op] ?? '科研状态操作',
    ...
    presentResult: (_args, result) => {
      ...
      const v = result.meta as StateOutput
